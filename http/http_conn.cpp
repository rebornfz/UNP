...
{
    MYSQL *mysql = NULL;
    connectionRAII mysqlcon(&mysql, connPool);//取一个mysql连接
...
int setnonblocking(int fd)
{
    ...
    int new_option = old_option | O_NONBLOCK;//或运算
    fcntl(fd, F_SETFL, new_option);//设置新的状态标记
    return old_option;
}
...
{//解析HTTP数据：将HTTP数据的每行数据提取出来，每行以回车\r和换行符\n结束  
    char temp;
    for(; m_checked_idx < m_read_idx; m_checked_idx++)
    {//m_checked_idx是当前正在解析的字节，m_read_idx是读缓冲区中已有的数据(客户端发送了多少HTTP请求数据到来),
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_version++ = '\0';
    ...
    {
        m_url += 7;
        //strchr(char *str,char character)
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if(cgi == 1 && (*(p + 1) == '2') || *(p + 1) == '3');
    {
        ...
        /*else if(m_SQLVerify == 1)//CGI校验，使用连接池
        {
            //注册
            if (*(p + 1) == '3')
            {
